...
<main x-data="ordersData()" class="flex-1 px-4 sm:px-6 md:px-10 py-8 overflow-auto">
    <!-- Header -->
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        ...
        <a href="/order"
            class="w-full sm:w-auto text-center bg-purple-600 text-white px-4 py-2 rounded-lg shadow hover:bg-purple-700 transition">
            + Add Order
        </a>
    </div>
...
<div x-data="{
        orders: [
            {id: 101, customer_name: 'John Doe', order_date: '2025-12-01', status: 'Pending', total: 'RS 1500', notes: 'Urgent delivery'},
            {id: 102, customer_name: 'Alice Smith', order_date: '2025-12-02', status: 'Completed', total: 'RS 2500', notes: 'Gift wrap required'},
            {id: 103, customer_name: 'Bob Johnson', order_date: '2025-12-03', status: 'Cancelled', total: 'RS 1200', notes: ''},
        ],
        filteredOrders() { return this.orders; },
        confirmDelete(id) { alert('Delete order ID ' + id); }
    }">
        ...
        <table class="w-full text-sm text-left border-collapse min-w-max">
            <thead class="bg-gray-100 text-gray-700 sticky top-0 z-10">
                <tr>
                    ...
                    <th class="p-3 border min-w-[150px]">Notes</th>
                    <th class="p-3 border text-center min-w-[120px]">Action</th>
                </tr>
            </thead>
            ...
            <tbody>
                <template x-for="order in filteredOrders()" :key="order.id">
                    <tr class="hover:bg-gray-50">
                        <td class="p-3 border" x-text="order.id"></td>
                        <td class="p-3 border" x-text="order.customer_name"></td>
                        <td class="p-3 border" x-text="order.order_date"></td>
                        <td class="p-3 border" x-text="order.status"></td>
                        <td class="p-3 border" x-text="order.total"></td>
                        <td class="p-3 border" x-text="order.notes"></td>
                                ...
                                <a :href="`/updateorder/${order.id}`"
                                   ...
                                   title="Edit Order">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" viewBox="0 0 20 20" fill="currentColor">
                                        ...
                                        <path fill-rule="evenodd" d="M2 15a1 1 0 011-1h2.586l7.293-7.293-2.586-2.586L3 11.414V15a1 1 0 01-1 1H2v-1z" clip-rule="evenodd"/>
                                    </svg>
                                </a>

                                <!-- Delete -->
                                <button @click.prevent="confirmDelete(order.id)" 
                                        class="text-red-600 hover:text-red-800" 
                                        title="Delete Order">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M8 9a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1zm-3-2a1 1 0 000 2h10a1 1 0 100-2H5zm-2 7a2 2 0 002 2h10a2 2 0 002-2V7H3v7zM4 4h12V3H4v1z" clip-rule="evenodd"/>
                                    </svg>
                                </button>
                            </div>
                        </td>
