...
<body class="h-screen flex bg-gray-50">
            ...
            <div class="relative w-full flex items-center gap-3">
                ...
                <div x-show="open" @click.away="open = false"
                    ...
                    <x-dropdown-link :href="route('profile.edit')"
                        class="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-t-lg">Profile</x-dropdown-link>
    ...
    <div class="flex-1 flex flex-col overflow-y-auto">
        ...
        <main class="flex-1 p-4 md:p-10 overflow-auto"
            x-data="{ selectedDesign: 'minimal', quotation: {{ json_encode($quotation) }} }">
                    ...
                    <div x-data="{ showPayment: false }">
                        ...
                        <div x-show="showPayment" x-transition
                            ...
                            <div class="bg-white w-96 p-6 rounded-lg shadow-lg relative">
                                ...
                                @if($remainingAmount <= 0) <div class="p-4 bg-green-100 text-green-800 rounded">
                                    Client has already fully paid.
                            </div>
                            @else
                            <form action="{{ route('payments.store', $quotation->id) }}" method="POST"
                                class="space-y-3">
                                @csrf
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="amount"
                                        ...
                                        placeholder="Enter payment amount" value="{{ $remainingAmount }}" min="1"
                                        max="{{ $remainingAmount }}" required>
                                    <small class="text-gray-500">Remaining amount:
                                        RS{{ number_format($remainingAmount, 2) }}</small>
                                </div>
                                <div>
    ...
    <div class="a4-container mx-auto bg-white rounded-xl shadow-xl p-8 lg:p-12 mb-10 a4-page">
        ...
        <div
            class="flex flex-col md:flex-row justify-between items-start md:items-center border-b-2 border-gray-200 pb-4 mb-8 gap-4">
            <div class="flex flex-col items-start gap-3">
                ...
                <!-- Company Details -->
                <div class="text-left">
                    <div class="text-lg font-bold text-purple-600">{{ $company->company_name ?? 'Your Company Name' }}
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div>
                <h3 class="font-semibold text-lg text-purple-600 mb-2 border-b border-purple-200 inline-block">Billed To
                </h3>
            ...
            <div class="text-right">
                <div class="mb-2"><span class="font-semibold text-gray-700">Quotation Date:</span> <span
                        class="font-medium">{{ $quotation->created_at->format('Y-m-d') }}</span></div>
                <div class="mb-2"><span class="font-semibold text-gray-700">Due Date:</span> <span
                        class="font-medium">{{ $quotation->due_date ?? 'N/A' }}</span></div>
                <div class="mb-2"><span class="font-semibold text-gray-700">Status:</span> <span
                        class="font-bold text-green-600">{{ $quotation->status }}</span></div>
            </div>
        </div>
