...
<div class="flex min-h-screen">
    ...
    <!-- Sidebar -->
    <aside class="w-60 bg-white shadow-md border-r flex flex-col justify-between h-screen">
                ...
                <form action="{{route('postupdateclient',$clients->id)}}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" name="name" class="w-full border p-2 rounded-lg mt-1" value="{{$clients->name}}">
                        </div>

                        <div>
                            <label class="block text-gray-700 font-medium">Company</label>
                            <input type="text" name="company" class="w-full border p-2 rounded-lg mt-1" value="{{$clients->company}}">
                        </div>

                        <div>
                            <label class="block text-gray-700 font-medium">Email</label>
                            <input type="email" name="email" class="w-full border p-2 rounded-lg mt-1" value="{{$clients->email}}">
                        </div>

                        <div>
                            <label class="block text-gray-700 font-medium">Phone</label>
                            <input type="text" name="phone" class="w-full border p-2 rounded-lg mt-1" value="{{$clients->phone}}">
                        </div>
                    </div>

                    <div class="mb-4">
                        <label class="block text-gray-700 font-medium">Address</label>
                        <textarea name="address" class="w-full border p-2 rounded-lg mt-1" rows="3">{{$clients->address}}</textarea>
                    </div>

                    <div class="mb-4">
                        <label class="block text-gray-700 font-medium">Notes (Optional)</label>
                        <textarea name="notes" class="w-full border p-2 rounded-lg mt-1" rows="3">{{$clients->notes}}</textarea>
                    </div>
